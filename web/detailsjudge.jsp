    ...
    layui.use(['layer'], function () {
        ...
        $('#confirmbay').on('click', function () {
            ...
            if (address.trim().length == 0) {
                ...
                <%--alert("${pageContext.requ
                est.contextPath}/buyheroservlet?c_id=${item.c_id}&buycount=${heroorders.buycount}&hero_skin=${heroorders.hero_skin}&JUDGEBUY=ok&o_shippingaddress="+address+"")--%>
                <%
                    String car_id = request.getParameter("car_id");
                    if(car_id!=null){
                        %>
                $(location).attr('href', "${pageContext.request.contextPath}/buyheroservlet?c_id=${item.c_id}&buycount=${heroorders.buycount}&hero_skin=${heroorders.hero_skin}&JUDGEBUY=ok&o_shippingaddress=" + address + "&car_id=<%=car_id%>");
