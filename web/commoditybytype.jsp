    ...
    <div class="commod-cont-wrap">
        <div class="commod-cont w1200 layui-clear">
            <div class="left-nav">
                ...
                <div class="list-box">
                    <c:forEach var="type" items="<%=request.getServletContext().getAttribute(RegionValue.HERO_COMMODITY_TYPE)%>"
                               varStatus="vars">
                        <dl>
                            <dt class="active" off="true">${type.type_name}</dt>
                            <c:set var="listcomm" value="${type.commodityList}"></c:set>
                            <dd style="display: none;" class="layui-bg-cyan"><a href="${pageContext.request.contextPath}/commtitytypeservlet?type_id=${type.type_id}">右侧展示</a></dd>
                            <c:forEach var="comm" items="${listcomm}">
                                <dd style="display: none;"><a href="">${comm.c_name}</a></dd>
                            </c:forEach>
                        </dl>
                    </c:forEach>
                </div>
            ...
            <div class="right-cont-wrap">
                <div class="right-cont">
                    ...
                    <div class="cont-list layui-clear" id="list-cont">
                        <c:set var="pagebean"
                               value="<%=request.getAttribute(RegionValue.HERO_COMMDITYBYTPYE_ID)%>"></c:set>
                        <c:set var="listcomm" value="${pagebean.list}"></c:set>
                        <c:forEach var="comm" items="<%=request.getAttribute(RegionValue.HERO_COMMDITYBYTPYE_ID)%>">
                            <div class="item">
                                <div class="img">
                    ...
                    <div style="text-align: center;">
                        <ul class="pagination">
                            <c:if test="${pagebean.pageNumber!=1}">
                                <li><a class="active"
                                       href="${pageContext.request.contextPath}/<%=request.getAttribute(RegionValue.SUB_COMMTITYSERVLE_NAME)%>?pageNumber=1">首页</a>
                                </li>
                                <li><a href="${pageContext.request.contextPath}/<%=request.getAttribute(RegionValue.SUB_COMMTITYSERVLE_NAME)%>?pageNumber=${pagebean.pageNumber-1}">上一页</a></li>
                            ...
                            <c:forEach begin="${pagebean.start}" end="${pagebean.end}" step="1" var="i">
                                ...
                                <c:if test="${pagebean.pageNumber!=i}">
                                    <li><a href="${pageContext.request.contextPath}/<%=request.getAttribute(RegionValue.SUB_COMMTITYSERVLE_NAME)%>?pageNumber=${i}">${i}</a></li>
                                </c:if>
                            </c:forEach>

                            <c:if test="${pagebean.pageNumber!=pagebean.totalPage}">
                                <li><a href="${pageContext.request.contextPath}/<%=request.getAttribute(RegionValue.SUB_COMMTITYSERVLE_NAME)%>?pageNumber=${pagebean.pageNumber+1}">下一页</a></li>
                                <li><a class="active"
                                       href="${pageContext.request.contextPath}/<%=request.getAttribute(RegionValue.SUB_COMMTITYSERVLE_NAME)%>?pageNumber=${pagebean.totalPage}">尾页</a>
                                </li>
                            </c:if>
                            <c:if test="${pagebean.pageNumber==pagebean.totalPage}">
